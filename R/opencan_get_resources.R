...
#' @param query_resultsThe results of a call to `opencan_search`
...
opencan_get_ressources <- function(record_id = NULL,
  ...
  if (!is.null(record_id)){
    ...
    resources_output <- as_tibble(results$resources)
    break
    ...
    if (inherits(query_results, "list")){
      ...
      resources_output <- as_tibble(query_results$resources)
